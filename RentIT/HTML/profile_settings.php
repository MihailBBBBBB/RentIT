...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!empty($_FILES['avatar']['name'])) {
    ...
    if (!$upload_dir || !is_dir($upload_dir)) {
        $error = 'Папка img не найдена. Путь: ' . htmlspecialchars(__DIR__ . '/../img');
    } elseif (!is_writable($upload_dir)) {
        $error = 'Папка img недоступна для записи: ' . $upload_dir;
    } else {
        ...
        if (in_array($ext, $allowed)) {
            ...
            if (move_uploaded_file($tmp, $target)) {
                ...
                $error = 'Ошибка при перемещении файла. Код: ' . $_FILES['avatar']['error'];
            }
        } else {
            $error = 'Недопустимый формат файла (jpg, png, gif, webp).';
        }
    }
}
      ...
      <header class="header">
        ...
        <div class="account">
          ...
          <button id="topupBtn" style="background-color:#22c55e;color:white;padding:8px 16px;
              border-radius:6px;border:1px solid #16a34a;cursor:pointer;font-weight:500;">Top up</button>
